...
float get_median_nn_via_merge( int a[], unsigned n, int b[], unsigned m )
{
	if (n == 0) return median(b, m);
	if (m == 0) return median(a, n);
    ...
    int prev, cur = -1;
    for (unsigned ct = 0, ax = 0, bx = 0; ct <= (n + m) / 2; ++ct) {
        prev = cur;
        if (a[ax] < b[bx]) {
            cur = a[ax];
            ++ax;
...
void median_driver( int a[], unsigned n, int b[], unsigned m ) {
    float merge_median = get_median_nn_via_merge(a, n, b, m);
/*
    cout << merge_median << "\n";
